...
<p><a href="..#add-a-party">Add a party</a>, add ‘buyer’ to its <code>.roles</code>, and set its <a href="#/CONTRACTING_BODY/ADDRESS_CONTRACTING_BODY/OFFICIALNAME">fields as above</a></p>
      ...
      <tr>
        ...
        <td>
<p>Discard. If there are multiple parties with the role ‘buyer’, then it is a joint procurement. (When <em>The contract involves joint procurement</em> is checked, at least one additional contracting authority is provided.)</p>
        </td>
      </tr>
      <tr>
        <td></td>
...
<p>Add ‘centralPurchasingBody’ to the <code>.roles</code> of the party <span class="badge badge-proposal">Proposal</span></p>
      ...
      <tr>
        ...
        <td>
<p>Discard. If no <code>tender.participationFees</code> have a <code>.type</code> of ‘document’, then access is assumed to be unrestricted.</p>
...
<p>Otherwise, <a href="..#add-a-party">add a party</a>, add ‘contactPoint’ to its <code>.roles</code>, and set its <a href="#/CONTRACTING_BODY/ADDRESS_CONTRACTING_BODY/OFFICIALNAME">fields as above</a>. <span class="badge badge-proposal">Proposal</span></p>
        </td>
      </tr>
      <tr>
        <td></td>
...
<p><em>This results in a loss of structure.</em> <span class="badge badge-warning">Attention</span></p>
      ...
      <tr>
        ...
        <td>
<p>Add ‘electronicSubmission’ to <code>tender.submissionMethod</code> (if not already added), and map to <code>tender.submissionMethodDetails</code></p>
  ...
  <table class="docutils">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
<p>Discard. If the notice is for prior information only, then this element is repeatable in <a href="http://simap.ted.europa.eu/documents/10184/99173/EN_F01.pdf">present forms</a>, but may not be in <a href="https://github.com/eForms/eForms/issues/55">future forms</a>.</p>
        </td>
      </tr>
      <tr>
        <td>II.1</td>
...
<p><em>This results in a loss of structure.</em> <span class="badge badge-warning">Attention</span></p>
      ...
      <tr>
        <td>II.2.5</td>
        <td id="/OBJECT_CONTRACT/OBJECT_DESCR/AC">
...
<p>Map to <code>.contractPeriod.endDate</code> <span class="badge badge-proposal">Proposal for lots</span></p>
      ...
      <tr>
        ...
        <td>
<p>If checked, add a <code>Renewals</code> object to the <code>tender.renewals</code> array, and set its <code>.id</code>. If the contract is divided into lots, set its <code>.relatedLot</code> to the lot’s <code>.id</code>. <span class="badge badge-proposal">Proposal</span></p>
        </td>
      </tr>
      <tr>
        <td></td>
...
<p>Set the renewals’ <code>.description</code> <span class="badge badge-proposal">Proposal</span></p>
      ...
      <tr>
        ...
        <td>
<p>If checked, add a <code>Variants</code> object to the <code>tender.variants</code> array, and set its <code>.id</code>. If the contract is divided into lots, set its <code>.relatedLot</code> to the lot’s <code>.id</code>. <span class="badge badge-proposal">Proposal</span></p>
...
<div class="wy-table-responsive">
  <table class="docutils">
    ...
    <tbody>
      ...
      <tr>
        <td></td>
        <td id="/LEFTI/PARTICULAR_PROFESSION/@CTYPE">
